...
| **Sanitization Metadata** | `sanitization` | Object describing the `status` (`"synthetic"`, `"sanitized"`, or `"real_approved"`), `method` used, `redaction_notes`, and a valid `review_approval` block for non-synthetic data. |
...
| **Label Description & Claims** | `claims` | Array of claim objects containing `claim_id` (prefixed with `claim-`), the claim text, `claim_type`, `linked_label_ids`, `linked_evidence_ids`, and `evidence_status` (`"linked"` or `"missing"`). |
...
| **Outliers & Impostors** | `outlier_impostor_candidates` | Array of candidate session/event objects inside `representative_sessions` flagged with `"outlier"` or `"impostor"`. |
...
| **Topology Coordinates** | `cluster.embedding_map` | Object containing `map_id`, `map_tool` (e.g., `"datamapplot"`), coordinate space bounds, and `coordinates` (`x` and `y` float values). |
...
6. **Preserve audit/provenance links**: When creating a new iteration of a `CasePackage`, carry forward the `refinement_id` and referenced `EvaluationReport` IDs in the new package's `provenance` metadata to ensure complete auditability of the refinement loop.
...
#     source_evaluation_report_ids=recipe["source_review_ids"]
# )
```
...
* **Approved / Synthetic / Sanitized Postures Only**: Packages must be explicitly classified into one of three postures:
  ...
  3. `real_approved`: Minimal real data permitted for specific review context.
...
* **Claim-to-Evidence Traceability**: Every claim generated by the labeling tool must map to one or more `evidence_id`s. Missing evidence links must fail validation.
